...
## Test Vector Setup
To process this input via a System Verilog simulation, it must be converted to a test vector. This can be done via the below command. Note: zstd uses little-endian format, and this command maintains that since it is word-aligned.

```
head -c 32 input.zst | xxd -p -c 2 > input.data
```

We can use xxd to print 2 bytes per line. This can then easily be read into a test bench or instruction memory module. For this part of the project, to confirm functionality we only process the first 32 bytes. This is guaranteed to contain the entire header, but obviously not the rest of the compressed data.

We can analyze this section of the test vector file and acquire the following table. Note, the first two lines are the "magic word" used to indicate that this a zstd compressed file. To see the little-endian storage format, note that the first line corresponds to the value 0xFD2FB528. In memory, this corresponds to 0x28B52FFD.
...
| 0x84                      | 0x58                  | 0 bytes                 | 0x00800000              |


We can then run the simulation and compare the results. We can see that all expected values are present. The waves in this photo can be generated by running `do Frame_Header.tcl` in the ModelSim terminal.
